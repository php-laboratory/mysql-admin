<?php
  class Mysql {
    ...
    function importTable($database, $table, $values) {
      $this->selectDatabase($database);
      ...
      foreach ($values as $rowIndex => $valueRow) {
        ...
        if ($rowIndex === 0) {
          foreach ($valueRow as $column => $value) {
            array_push($columnNames, $column);
          }
        }

        foreach ($columnNames as $columnName) {
          array_push($row, "\"" . $valueRow[$columnName] . "\"");
        }
      ...
      return $this->execute(array(
        ...
        "INSERT INTO `$table` (" . join(", ", $columnNames) . ") VALUES " . join(", ", $rows) . ""
      ));
    }
    ...
    function getTableCreateStatement($database, $table) {
      $this->selectDatabase($database);
